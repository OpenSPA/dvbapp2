...
defaultPaths = {
	...
	SCOPE_PICON: (eEnv.resolve("${datadir}/enigma2/picon/"), PATH_CREATE),
...
def getRecordingFilename(basename, dirname=None):
	...
	filenameBytes = filename.encode(encoding="UTF-8", errors="ignore")
	filename = filenameBytes[:243].decode(encoding="UTF-8", errors="ignore")
	if dirname is None:
...
def isPluginInstalled(pluginName, pluginFile="plugin", pluginType=None):
	...
	for type in types:
		for extension in ["c", ""]:
			if isfile(join(scopePlugins, type, pluginName, f"{pluginFile}.py{extension}")) or isfile(join(scopePlugins, type, pluginName, f"{pluginFile}.so")):
				return True
	return False
