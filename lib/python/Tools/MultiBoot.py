...
from Tools.Directories import SCOPE_CONFIG, copyfile, fileHas, fileReadLine, fileReadLines, fileWriteLines, resolveFilename, pathExists
	...
	def bootDeviceMounted(self, data, retVal, extraArgs):  # Part of activateSlot().
		if retVal:
			...
			if self.bootCode == "12" and "BOXMODE" not in startup:
				open(join(self.tempDir, target), "w").write(bootSlot["cmdline"]["12"])
