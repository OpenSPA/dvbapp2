...
class InitInputDevices:
	...
	def setupConfigEntries(self, device):
		...
		if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius', 'evo', 'galaxym6'):
			...
			data = 100
		configItem.repeat = ConfigSlider(default=BoxInfo.getItem("RemoteRepeat", data), increment=10, limits=(0, 500))
		configItem.repeat.addNotifier(self.inputDevicesRepeatChanged)
		if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius', 'evo', 'galaxym6'):
			data = 200
		else:
			data = 700
		configItem.delay = ConfigSlider(default=BoxInfo.getItem("RemoteDelay", data), increment=100, limits=(0, 5000))
