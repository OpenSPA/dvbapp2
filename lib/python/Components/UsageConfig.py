...
from Components.config import ConfigBoolean, ConfigClock, ConfigDirectory, ConfigDictionarySet, ConfigFloat, ConfigInteger, ConfigIP, ConfigLocations, ConfigNumber, ConfigSelectionNumber, ConfigPassword, ConfigSelection, ConfigSet, ConfigSlider, ConfigSubsection, ConfigText, ConfigYesNo, NoSave, config, ConfigDirectory
from Components.Harddisk import harddiskmanager
from Components.NimManager import nimmanager
from Components.ServiceList import refreshServiceList
from Components.SystemInfo import BoxInfo
from Tools.Directories import SCOPE_HDD, SCOPE_SYSETC, SCOPE_TIMESHIFT, SCOPE_PICON, defaultRecordingLocation, fileContains, isPluginInstalled, resolveFilename
...
def InitUsageConfig():
	...
	config.usage.quality_SDicon.addNotifier(refreshServiceList)
	config.usage.quality_HDicon = ConfigYesNo(default = True)
	config.usage.quality_HDicon.addNotifier(refreshServiceList)
	config.usage.quality_4Kicon = ConfigYesNo(default = True)
	config.usage.quality_4Kicon.addNotifier(refreshServiceList)
	config.usage.quality_3Dicon = ConfigYesNo(default = True)
	config.usage.quality_3Dicon.addNotifier(refreshServiceList)
	config.usage.quality_IPTVicon = ConfigYesNo(default = True)
	config.usage.quality_IPTVicon.addNotifier(refreshServiceList)
	##########################################################################################
	...
	# OPENSPA [morser] picon_path modification
	...
	config.epg.items_in_event_ext_descr = ConfigYesNo(default=True)
	...
	def EpgCacheChanged(configElement):
		...
		if not config.misc.epgcache_filename.value.startswith("/etc/enigma2/"):
			...
			if exists(epgCachePath):
				remove(epgCachePath)
#	config.misc.allowed_epgcachepath = ConfigLocations(default = ['/etc/enigma2/'])
...
#	harddiskmanager.on_partition_list_change.append(partitionListChanged)
	...
	crashlogheader = _("We are really sorry. Your receiver encountered "
		...
		"Please send the logfile %senigma2_crash_xxxxxx.log to https://openspa.info.\n"
	...
	if isPluginInstalled("spazeMenu"):
		choices.append(("spaguide",_("Show OpenSPA Guide")))
		choices.append(("spasingle",_("Show OpenSPA Single EPG")))
		default = "spasingle"
	else:
		default = "pliepg"
	config.plisettings.PLIEPG_mode = ConfigSelection(default=default, choices=choices)
	choices = [
		("eventview", _("Show EventView")),
		("epgpress", _("Show EPG")),
		("single", _("Show Single EPG"))
	...
	if isPluginInstalled("spazeMenu"):
		choices.append(("spaguide",_("Show OpenSPA Guide")))
		choices.append(("spasingle",_("Show OpenSPA Single EPG")))
	config.plisettings.PLIINFO_mode = ConfigSelection(default="eventview", choices=choices)
	########################################################
	#### OPENSPA [morser] New selections for buttons
	config.usage.leftrightmode = ConfigSelection(default="0", choices=[
		("0", _("Zap in Channel list")),
		("1", _("Zap in History")),
		("2",_("InfoBar EPG"))
	])
	choices = [
		("timers",_("RecordTimers")),
		("videoset",_("Video Settings")),
		("audioset",_("Audio Settings")),
		("infobarepg",_("InfoBar EPG")),
		("movies", _("Show Movies")),
		("resolution", _("Change Aspect/Resolution"))
		]
	if isPluginInstalled("Kodi"):
		choices.append(("kodi", _("Kodi")))
	if isPluginInstalled("spazeMenu"):
		choices.append(("camdmanager", _("CAMD Manager")))
		choices.append(("meteo", _("InfoMeteo"))),
		choices.append(("mediacenter", _("MediaCenter"))),
		choices.append(("panelextra", _("Extra Panel")))

	config.usage.bluelong = ConfigSelection(default="videoset", choices=choices)
	config.usage.redlong = ConfigSelection(default="timers", choices=choices)
