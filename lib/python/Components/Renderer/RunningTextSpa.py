...
class RunningTextSpa(Renderer):
	def __init__(self):
		...
		self.X = self.Y = self.W = self.H = self.mStartDelay = self.mEndDelay= 0
			...
			for (attrib, value) in self.skinAttributes:
				if attrib == "font":
					...
					for o in options:
						...
						elif opt == "backtime" and val:
							self.backtime = retValue(val, 0, self.backtime)
						elif opt == "startdelay" and val:
							self.mStartDelay = retValue(val, 0, self.mStartDelay)
						elif opt == "enddelay" and val:
							self.mEndDelay = retValue(val, 0, self.mEndDelay)
...
#		self.halign =		0 - LEFT; 1 - RIGHT;   2 - CENTER;   3 - BLOCK
	...
	def movingLoop(self):
		if self.A <= self.P <= self.B:
			...
			if self.type == RUNNING:
				if self.P < self.A:
					self.P = self.B + abs(self.mStep+self.addstep)
				else:
					self.P = self.A - abs(self.mStep+self.addstep)
			else:
				if (self.direction == TOP or self.direction == LEFT):
					if self.backtime>0:
						if self.mStep<=0:
							self.addstep=self.backtime
						else:
							self.addstep=0
					self.mStep = -(self.mStep)
					if self.P+(self.mStep+self.addstep)>(self.mStep+self.addstep):
						self.P=0
						self.addstep=0
				else:
					self.mStep = -(self.mStep)
