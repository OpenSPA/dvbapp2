...
class FlashManager(Screen, HelpableScreen):
	...
	def keyOk(self):
		...
		if currentSelection[0][1] == "Expanded":
			...
			self.session.openWithCallback(self.reloadImagesList, FlashImage, currentSelection[0][0], currentSelection[0][1], self.destpath)
...
class FlashImage(Screen, HelpableScreen):
	...
	def flashImage(self):
		...
		if imageFiles:
			...
			if bootSlots:
				mtdKernel = bootSlots[self.slotCode]["kernel"].split(sep)
