...
class NcamInfo:
	...
	def confPath(self):
		...
		if fileExists("/tmp/.ncam/ncam.version"):
			...
			for i in data:
				...
				elif "web interface support:" in i.lower() or "webif support:" in i.lower():
	...
	def getUserData(self):
		...
		port = "0"
		ipconfigured = "no"
		[ncamconf, ipcompiled] = self.confPath()
		ret = _("file ncam.conf could not be found")
		if ncamconf is not None:
			# Assume both errors at once
			ret = _("ncam webif disabled")
			if ipcompiled == "yes":
				ret = ret + ", " + _("ncam webif blocking localhost (::1 and 127.0.0.1)")
			else:
				ret = ret + ", " + _("ncam webif blocking localhost (127.0.0.1)")
			user = pwd = ""
			data = open(ncamconf, "r").readlines()
			for i in data:
...
class NcamInfoMenu(Screen):
	...
	sizeY = int(550*f)
	skin="""
	<screen name="NcamInfoMenu" position="center,center" size="%s,%s">
...
class NcamInfoConfigScreen(Screen, ConfigListScreen):
	...
	<screen name="NcamInfoConfigScreen" position="center,center" size="%s,%s">
		<widget name="config" position="0,0" size="%s,%s" scrollbarMode="showOnDemand" transparent="0"/>
		<widget name="status" position="15,%s" size="%s,%s" transparent="0"/>
		<ePixmap pixmap="buttons/red.png" position="0,e-40" size="%s,%s" alphatest="blend"/>
		<widget source="key_red" render="Label" position="0,e-40" zPosition="1" size="%s,%s" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1"/>
		<ePixmap pixmap="buttons/green.png" position="%s,e-40" size="%s,%s" alphatest="blend"/>
	...
	def __init__(self, session, msg = None):
		...
		self["config"] = ConfigList(self.ncamconfig)
	...
	def elementChanged(self, instance):
		self.createSetup()
		try:
			self["config"].l.setList(self.ncamconfig)
		except KeyError:
			pass
	...
	def createSetup(self):
		self.ncamconfig = []
		self.ncamconfig.append(getConfigListEntry(_("Read Userdata from ncam.conf"), config.ncaminfo.userdatafromconf))
