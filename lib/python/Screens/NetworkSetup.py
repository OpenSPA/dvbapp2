...
class Networksimpleconf(Screen):
	...
	def create(self, value=1):
		try:
			if value:
				self.Console.ePopen("/usr/bin/setup_openvpn.sh ")
		except IOError:
...
class NetworkUdpxy(Screen):
	...
	def UdpxyStartStop(self):
		commands = []
		if fileExists('/etc/init.d/udpxy'):
			if self.my_udpxy_run:
				commands.append('/etc/init.d/udpxy stop; killall -9 udpxy')
			else:
				commands.append('/bin/su -l -c "/etc/init.d/udpxy start"')
			self.Console.eBatch(commands, self.StartStopCallback, debug=True)
		else:
	...
	def activateUdpxy(self):
		...
		if fileExists('/etc/init.d/udpxy'):
			if fileExists('/etc/rc3.d/S20udpxy'):
				commands.append('update-rc.d -f udpxy remove')
			else:
				commands.append('update-rc.d -f udpxy defaults 20')
			self.Console.eBatch(commands, self.StartStopCallback, debug=True)
		else:
			self.session.open(MessageBox, _("Sorry! udpxy it was not found"), MessageBox.TYPE_INFO, timeout = 5)
