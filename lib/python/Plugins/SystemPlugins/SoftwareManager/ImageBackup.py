...
class ImageBackup(Screen):
	...
	def doFullBackupCB(self):
		...
		if SystemInfo["canRecovery"] and self.RECOVERY:
			cmdlist.append('7za a -r -bt -bd %s/%s-%s.%s-%s-backup-%s_recovery_emmc.zip %s/*' %(self.DIRECTORY, self.IMAGEDISTRO, self.DISTROVERSION, self.IMAGEBUILD, self.MODEL, self.DATE, self.MAINDESTROOT))
		elif SystemInfo["HasRootSubdir"]:
		...
		if SystemInfo["canMultiBoot"] and not self.RECOVERY and not SystemInfo["HasRootSubdir"]:
			...
			if SystemInfo["canRecovery"] and self.RECOVERY:
				cmdlist.append('echo "' + _("Image created on: %s/%s-%s.%s-%s-backup-%s_recovery_emmc.zip") %(self.DIRECTORY, self.IMAGEDISTRO, self.DISTROVERSION, self.IMAGEBUILD, self.MODEL, self.DATE) + '"')
			elif SystemInfo["HasRootSubdir"]:
