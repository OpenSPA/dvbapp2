...
class ImageBackup(Screen):
	...
	def doFullBackupCB(self):
		...
		## OPENSPA [morser] Add build revision in name ############################
		if isinstance(self.IMAGEBUILD, int):
			revision = ".%03d" % int(self.IMAGEBUILD) if self.IMAGEBUILD else ""
		else:
			revision = "." + str(self.IMAGEBUILD)
		cmdlist.append("echo 3 > /proc/sys/vm/drop_caches")  # Clear Memory
		cmdlist.append("7za a -r -bt -bd %s/%s-%s%s-%s-backup-%s_%s.zip %s/*" % (self.DIRECTORY, self.DISTRO, self.DISTROVERSION, revision, self.MODEL, self.DATE, iname, self.MAINDESTROOT))
		###########################################################################
		...
		if MultiBoot.canMultiBoot() and not self.RECOVERY and self.ROOTFSSUBDIR == "none":
			cmdlist.append(self.makeEcho(_("Multiboot Image created on: %s/%s-%s%s-%s-backup-%s_usb.zip") % (self.DIRECTORY, self.DISTRO, self.DISTROVERSION, revision, self.MODEL, self.DATE)))
			...
			cmdlist.append(self.makeEcho(_("Image created on: %s/%s-%s%s-%s-backup-%s_%s.zip") % (self.DIRECTORY, self.DISTRO, self.DISTROVERSION, revision, self.MODEL, self.DATE, iname)))
			cmdlist.append(self.makeEchoLine(True))
			cmdlist.append(self.makeSpace())
			cmdlist.append(self.makeEcho(_("Please wait...almost ready! ")))
			cmdlist.append(self.makeSpace())
