...
def InitConfig():
	...
	BACKUPFILES = ["/etc/enigma2/", "/etc/CCcam.cfg", "/usr/keys/",
		...
		"/etc/enigmalight.conf", "/etc/enigma2/volume.xml", "/etc/enigma2/ci_auth_slot_0.bin", "/etc/enigma2/ci_auth_slot_1.bin", "/etc/PrivateKey.key", "/etc/wg_token.key", "/etc/.ActiveCamd", "/etc/.CamdReStart.sh", "/etc/.CamdStart.sh", "/etc/keys",
		...
		+ eEnv_resolve_multi("${bindir}/oscam*")\
		+ eEnv_resolve_multi("${bindir}/ncam*")\
		+ eEnv_resolve_multi("${bindir}/CCcam*")\
