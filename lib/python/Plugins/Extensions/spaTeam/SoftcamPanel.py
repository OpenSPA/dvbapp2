...
from . import _
from Components.config import config, ConfigSubsection, ConfigText, configfile, getConfigListEntry, ConfigSelection, ConfigYesNo
...
config.plugins.showspateamextensions = ConfigYesNo(default=False)

config.plugins.spateam_frozencheck = ConfigSubsection()
config.plugins.spateam_frozencheck.list = ConfigSelection([('0',_("Off")),('1',_("1 min.")), ('5',_("5 min.")),('10',_("10 min.")),('15',_("15 min.")),('30',_("30 min."))])
...
if not Check_Softcam() and config.plugins.showspateamextensions.value:
	config.plugins.showspateamextensions.value = False
	config.plugins.showspateamextensions.save()
...
class SoftcamPanel(ConfigListScreen, Screen):
	def __init__(self, session):
		...
		self.partyfeed = os.path.exists("/etc/opkg/secret-feed.conf") or os.path.exists("/etc/opkg/secret-feed.conf")
		if self.partyfeed:
...
class ShowSoftcamPackages(Screen):
	...
	def setStatus(self,status = None):
		if status:
			...
			if status == 'update':
				statuspng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, "Extensions/spaTeam/icons/upgrade.png"))
			...
			if status == 'list':
				...
				statuspng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, "Extensions/spaTeam/icons/remove.png"))
	...
	def buildEntryComponent(self, name, version, description, state):
		...
		if not description:
			description = ""
		installedpng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, "Extensions/spaTeam/icons/installed.png"))
...
class ShowSoftcamPanelExtensions(ConfigListScreen, Screen):
	...
	def createSetup(self):
		self.editListEntry = None
		self.list = []
		self.list.append(getConfigListEntry(_("Show Softcam-Panel in Extensions Menu"), config.plugins.showspateamextensions))
		...
		self.list.append(getConfigListEntry(_("Frozen Cam Check"), config.plugins.spateam_frozencheck.list))
		self.list.append(getConfigListEntry(_("Wait time before start Cam 2"), config.softcam.waittime))
