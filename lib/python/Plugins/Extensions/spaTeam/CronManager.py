...
class SetupCronConf(Screen, ConfigListScreen):
	skin = """
		<screen position="center,center" size="560,400" title="Cron Manager">
			...
			<ePixmap pixmap="skin_default/buttons/blue.png" position="250,350" size="140,40" alphatest="on" />
			<widget name="key_blue" position="250,350" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1" />
		</screen>"""
	...
	def createSetup(self):
		...
		if config.spateam.cronmanager_runwhen.value != 'Hourly':
			self.list.append(getConfigListEntry(_("Time to execute command or script"), config.spateam.cronmanager_cmdtime))
		if config.spateam.cronmanager_runwhen.value == 'Weekly':
			self.list.append(getConfigListEntry(_("What Day of week ?"), config.spateam.cronmanager_dayofweek))
		if config.spateam.cronmanager_runwhen.value == 'Monthly':
			self.list.append(getConfigListEntry(_("What Day of month ?"), config.spateam.cronmanager_dayofmonth))
		self.list.append(getConfigListEntry(_("Command type"), config.spateam.cronmanager_commandtype))
		if config.spateam.cronmanager_commandtype.value == 'custom':
			self.list.append(getConfigListEntry(_("Command To Run"), config.spateam.cronmanager_user_command))
		else:
			self.list.append(getConfigListEntry(_("Command To Run"), config.spateam.cronmanager_predefined_command))
	...
	def checkentry(self):
		msg = ''
		if (config.spateam.cronmanager_commandtype.value == 'predefined' and config.spateam.cronmanager_predefined_command.value == '') or config.spateam.cronmanager_commandtype.value == 'custom' and config.spateam.cronmanager_user_command.value == '':
	...
	def saveMycron(self):
		hour = '%02d' % config.spateam.cronmanager_cmdtime.value[0]
		minutes = '%02d' % config.spateam.cronmanager_cmdtime.value[1]
		if config.spateam.cronmanager_commandtype.value == 'predefined' and config.spateam.cronmanager_predefined_command.value != '':
			command = config.spateam.cronmanager_predefined_command.value
		else:
			command = config.spateam.cronmanager_user_command.value

		if config.spateam.cronmanager_runwhen.value == 'Hourly':
			newcron = minutes + ' ' + ' * * * * ' + command.strip() + '\n'
		elif config.spateam.cronmanager_runwhen.value == 'Daily':
			newcron = minutes + ' ' + hour + ' * * * ' + command.strip() + '\n'
		elif config.spateam.cronmanager_runwhen.value == 'Weekly':
			if config.spateam.cronmanager_dayofweek.value == 'Sunday':
				newcron = minutes + ' ' + hour + ' * * 0 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Monday':
				newcron = minutes + ' ' + hour + ' * * 1 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Tuesday':
				newcron = minutes + ' ' + hour + ' * * 2 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Wednesday':
				newcron = minutes + ' ' + hour + ' * * 3 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Thursday':
				newcron = minutes + ' ' + hour + ' * * 4 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Friday':
				newcron = minutes + ' ' + hour + ' * * 5 ' + command.strip() + '\n'
			elif config.spateam.cronmanager_dayofweek.value == 'Saturday':
				newcron = minutes + ' ' + hour + ' * * 6 ' + command.strip() + '\n'
		elif config.spateam.cronmanager_runwhen.value == 'Monthly':
			newcron = minutes + ' ' + hour + ' ' + str(config.spateam.cronmanager_dayofmonth.value) + ' * * ' + command.strip() + '\n'
		else:
			command = config.spateam.cronmanager_user_command.value
		...
		out.close()
		rc = system('crontab /etc/cron/crontabs/root -c /etc/cron/crontabs')
		config.spateam.cronmanager_predefined_command.value = 'None'
		config.spateam.cronmanager_user_command.value = 'None'
		config.spateam.cronmanager_runwhen.value = 'Daily'
		config.spateam.cronmanager_dayofweek.value = 'Monday'
		config.spateam.cronmanager_dayofmonth.value = "1"
		config.spateam.cronmanager_cmdtime.value, mytmpt = ([0, 0], [0, 0])
		self.close()
