...
class Swap(Screen):
	...
	def getSwapDevice(self):
		...
		if path.exists('/etc/rcS.d/S98SwapManager'):
			...
			config.plugins.spateam.swapautostart.value = True
			config.plugins.spateam.swapautostart.save()
	...
	def updateSwap2(self, result=None, retval=None, extra_args=None):
		...
		if config.plugins.spateam.swapautostart.value and self.swap_place:
			self['autostart_off'].hide()
			self['autostart_on'].show()
		else:
			config.plugins.spateam.swapautostart.value = False
			config.plugins.spateam.swapautostart.save()
			configfile.save()
			self['autostart_on'].hide()
			self['autostart_off'].show()
